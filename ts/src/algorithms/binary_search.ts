 ...
 *  This algorithm represents the binary search algorithm implemented
    ...
    This algorithm assumes the simplest case of binary search
    searching over an explicit sorted array, but the search space can
    be any space that is sorted. For example, binary search can be used on
    functions which are monotonically increasing or decreasing in order to find
    the lowest possible value some condition is met (low bound binary search),
    or the highest possible value some condition is met (high bound
    binary search). 
    
    This means binary searching over the inputs of these functions to
    determine which input produces the most optimal value -- highest or
    lowest output value that meets some condition. 
